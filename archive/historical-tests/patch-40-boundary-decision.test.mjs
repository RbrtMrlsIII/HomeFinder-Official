...
const dictionary = JSON.parse(fs.readFileSync(`${root}docs/dictionary/domains/capability-boundaries.dictionary.json`, 'utf8'));
...
const byId = Object.fromEntries(dictionary.boundaries.map(x => [x.id, x]));
assert.equal(byId['reports.opsModeration'].status, 'resolved-broad-ops');
assert.deepEqual(byId['reports.opsModeration'].effectiveRoles, ['staff','moderator','admin']);
assert.equal(byId['kycReferenceIndex.opsRead'].status, 'resolved-admin-only');
assert.deepEqual(byId['kycReferenceIndex.opsRead'].effectiveRoles, ['admin']);
