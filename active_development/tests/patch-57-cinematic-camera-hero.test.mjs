...
test('Patch 57 A.2.2: Home does not depend on retired generated poster assets', () => {
  const world = read('data/cinematic-worlds.json');
  const scenes = read('data/cinematic-scenes.json');
  assert.doesNotMatch(world, /world01-(concept|clean-cinematic|poster)\.png/);
  assert.doesNotMatch(scenes, /world01-(concept|clean-cinematic|poster)\.png/);
  assert.doesNotMatch(world, /Meshy/);
});
