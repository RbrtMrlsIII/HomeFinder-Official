...
export const CATEGORY_OF_CLASSIFICATION = {
...
export const CATEGORIES = ["bedspace", "residential", "commercial", "industrial"];

export function categoryOf(classification){
...
export const MONTHLY_POINTS_BY_CLASSIFICATION = {
...
export const SALE_COMPLETION_BONUS = 300;
 ...
 */
export const LEASE_RTO_SUCCESS_BONUSES = [
...
export function monthlyPointsForClassification(classification){
    return MONTHLY_POINTS_BY_CLASSIFICATION[classification] || 10;
}

export function leaseRtoSuccessBonus(yearsOwned){
 ...
 */
export function contractPoints(opts){
...
export const OWNER_SATISFACTION_WEIGHT = 10;
...
export const BROKER_RESCUE_BONUS = 10;
...
export const DURATION_POINT_MONTHS = 1;
export const RENT_TO_OWN_SUCCESS_BONUS = 100; // minimum RTO success (<2y)

/* -------- 🔧 DEFAULT map radius (km) by tier index -------- */

export const RADIUS_BY_TIER_KM = [2, 4, 6, 9, 12, 15];

/* -------- 🔧 DEFAULT relocation cooldown (hours) by tier index -------- */

export const COOLDOWN_BY_TIER_HOURS = [72, 48, 24, 12, 6, 3];
...
export const BASE_WANTED_ACTIVE_CAP = 1;

/** Base owner listing cap with no listing boost. */
export const BASE_OWNER_LISTING_CAP = 1;

/** Base max images per listing with no listing boost. */
export const BASE_IMAGES_PER_LISTING = 1;
 ...
 * @deprecated
 */
export const EXTRA_LISTING_SLOT = {
    pricePhp: 0,
    slotsPerPurchase: 0,
    durationDays: 0,
 ...
 */
export const SEEKER_BOOST_PACKAGES = {
 ...
 */
export const OWNER_BOOST_PACKAGES = {
...
export function seekerTierForCategory(points){
 ...
 */
export function ownerTierForScore(completedContractsOrPoints, avgSatisfaction, totalPoints){
...
export function brokerTierForScore(satisfiedClosuresOrPoints, rentToOwnRescues, totalPoints){
    const points = totalPoints != null
        ? totalPoints
...
export function aggregateBrokerScore(byProperty){
...
export function brokerTierForProperties(byProperty){
...
export function durationPoints(monthsSustained, classification = "bedspace_dorm"){
...
export function tierIndexFromPoints(points, ladder) {
...
export function isBoostSliceActive(roleBoost){
...
export function resolveBoostPackageId(roleBoost){
 ...
 */
export function radiusForTier(tierIndex, seekerPackageId = 0){
    const base = RADIUS_BY_TIER_KM[tierIndex] ?? RADIUS_BY_TIER_KM[0];
    const bonus = seekerBoostPackage(seekerPackageId).radiusBonusKm;
    return base + bonus;
}

export function cooldownHoursForTier(tierIndex, seekerPackageId = 0){
...
export function totalImagesPerListing(tierIndex, ownerPackageId) {
 ...
 */
/**
 * Server/client-safe subscription entitlement check. The entitlement document is
 * the effective premium-access authority; an active flag alone is insufficient
 * when startsAt/endsAt establish a bounded grant.
 */
export function isSubscriptionEntitlementActive(entitlement){
...
export function activeExtraListingSlots(extraListings){
...
export function listingSlotsFromTier(tierIndex = 0){
 ...
 */
export function totalListingCap(ownerPackageId = 0, extraListings = null, tierIndex = 0, subscriptionEntitlement = null){
...
export const ACCEPTED_ID_LABELS = {
...
export const BROKER_LICENSE_TYPES = ["prc_license", "broker_certificate"];
 ...
 */
export function hasVerifiedId(profile){
...
export function hasBrokerLicense(profile){
...
export function canCreateFirstListing(profile, existingListingCount){
 ...
 */
export function activeBoostPackageIds(roleBoost, { pinOnly = false } = {}) {
    const ids = new Set();
    const raw = roleBoost || {};
...
export function pinBonusForActivePackages(roleBoost) {
...
export function pinSlotKeysForBoosts(roleBoost, line = "seeker") {
    return activeBoostPackageIds(roleBoost, { pinOnly: true }).map(id => `${line}-${id}`);
}

export function pinBonusForSeekerPackage(packageId){
    return Math.max(0, Number(seekerBoostPackage(packageId).pinBonus) || 0);
}
export function pinBonusForOwnerPackage(packageId){
 ...
 */
export function maxPinsForAccount({
    role = "seeker",
    tierIndex = 0,
    seekerPackageId = 0,
    ownerPackageId = 0,
